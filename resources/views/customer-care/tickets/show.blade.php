...
<head>
    ...
    <title>Ticket Details - Customer Care</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0"
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('customer-care.tickets.index') }}" class="flex items-center space-x-3 px-4 py-3 text-white purple-gradient rounded-lg font-medium transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <div class="flex items-center justify-between px-6 py-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <h1 class="text-xl font-bold text-white">Ticket Details</h1>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        <!-- Notification Icon -->
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                <!-- Back Button -->
                <div class="mb-6">
                    <a href="{{ route('customer-care.tickets.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full
                                        ...
                                        @elseif($ticket->status === 'escalated') bg-orange-100 text-orange-800
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-900">{{ $ticket->resolved_at->format('M d, Y h:i A') }}</p>
                                </div>
                                @endif
                                <div>
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Created</p>
                                    <p class="text-sm text-gray-900">{{ $ticket->created_at->format('M d, Y h:i A') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            <div class="mb-4 pb-4 border-b border-gray-200 flex items-center justify-between">
                                ...
                                @if($ticket->user->phone || $ticket->user->email)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-900">{{ $ticket->user->email }}</p>
                                </div>
                                @endif
                                @if($ticket->user->phone)
                                <div>
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Phone</p>
                                    <p class="text-sm text-gray-900">{{ $ticket->user->phone }}</p>
                                </div>
                                @endif
                                <div>
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Patient Record</p>
                                    <a href="{{ route('customer-care.customers.show', $ticket->user) }}" class="text-sm text-purple-600 hover:text-purple-800">
                                ...
                                <div class="border-l-4 border-orange-500 pl-4 py-2">
                                    <div class="flex items-center justify-between mb-1">
                                        <p class="text-xs font-medium text-gray-700">
                                            Escalated by: {{ $escalation->escalatedBy->name ?? 'Unknown' }}
                                            <span class="ml-2 px-1.5 py-0.5 text-[10px] bg-orange-100 text-orange-600 rounded">{{ ucfirst($escalation->status) }}</span>
                                        </p>
                                        <p class="text-xs text-gray-500">{{ $escalation->created_at->format('M d, Y h:i A') }}</p>
                                    </div>
                                    <p class="text-sm text-gray-900">{{ $escalation->reason }}</p>
                                </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        <!-- Quick Contact -->
                        @if($ticket->user && ($ticket->user->phone || $ticket->user->email))
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Quick Contact</h3>
                            <div class="space-y-2">
                                @if($ticket->user->phone)
                                @php
                                    $whatsappPhone = preg_replace('/[^0-9+]/', '', $ticket->user->phone);
                                ...
                                @endif
                                @if($ticket->user->email)
                                <a href="mailto:{{ $ticket->user->email }}?subject=Re: {{ urlencode($ticket->subject) }}" 
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            ...
                            <div class="space-y-3">
                                ...
                                <form method="POST" action="{{ route('customer-care.tickets.update-status', $ticket) }}" id="updateStatusForm">
                                    ...
                                    <button type="submit" class="w-full px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-semibold">
                                        Update Status
                                    </button>
                                </form>
                                @endif
                                <a href="{{ route('customer-care.escalations.create-from-ticket', $ticket) }}" class="block w-full px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm font-semibold text-center">
                                    Escalate
                                </a>
                            </div>
                        </div>
                        ...
                        @if($ticket->agent)
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Assigned Agent</h3>
                            <div class="space-y-2">
                                <p class="text-sm font-medium text-gray-900">{{ $ticket->agent->name }}</p>
                                <p class="text-xs text-gray-500">{{ $ticket->agent->email }}</p>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Resolved By</h3>
                            <div class="space-y-2">
                                <p class="text-sm font-medium text-gray-900">{{ $ticket->resolvedBy->name }}</p>
                                <p class="text-xs text-gray-500">{{ $ticket->resolvedBy->email }}</p>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400">{{ $ticket->created_at->format('h:i A') }}</p>
                                </div>
                                @if($ticket->updated_at && $ticket->updated_at != $ticket->created_at)
                                <div>
                                    <p class="text-xs text-gray-500">Last Updated</p>
                                    <p class="text-sm text-gray-900">{{ $ticket->updated_at->format('M d, Y') }}</p>
                                    <p class="text-xs text-gray-400">{{ $ticket->updated_at->format('h:i A') }}</p>
                                </div>
                                @endif
                                @if($ticket->resolved_at)
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400">{{ $ticket->resolved_at->format('h:i A') }}</p>
                                </div>
                                @endif
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full" onclick="event.stopPropagation()">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-white/90 mt-0.5">{{ $ticket->user->name }}</p>
                        </div>
                    </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    @if($ticket->user->phone)
                    @php
                        $modalWhatsappPhone = preg_replace('/[^0-9+]/', '', $ticket->user->phone);
                    ...
                    <a href="https://wa.me/{{ $modalWhatsappPhone }}" 
                       ...
                       class="flex items-center gap-3 w-full p-4 bg-green-50 border-2 border-green-200 rounded-lg hover:bg-green-100 hover:border-green-300 transition-all group">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-600">{{ $ticket->user->phone }}</p>
                        </div>
                    ...
                    <a href="mailto:{{ $ticket->user->email }}?subject=Re: {{ urlencode($ticket->subject) }}" 
                       ...
                       class="flex items-center gap-3 w-full p-4 bg-purple-50 border-2 border-purple-200 rounded-lg hover:bg-purple-100 hover:border-purple-300 transition-all group">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-600 truncate">{{ $ticket->user->email }}</p>
                        </div>

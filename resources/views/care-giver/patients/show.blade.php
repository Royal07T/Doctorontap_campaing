...
<head>
    ...
    <title>{{ $patient->name }} - Patient Profile</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="purple-gradient shadow-lg z-10">
                <div class="flex items-center justify-between px-6 py-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-white">{{ $patient->name }}</h1>
                            <p class="text-sm text-purple-200">Patient Profile</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('care_giver.patients.index') }}" class="text-white hover:text-purple-200 text-sm font-medium">
                            ← Back to Patients
                        </a>
                    </div>
                </div>
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                <div class="max-w-7xl mx-auto space-y-6">
                    @if(session('success'))
                    <div class="p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 rounded">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h2 class="text-lg font-bold text-gray-900 mb-4">Patient Information</h2>
                    ...
                    <!-- Active Care Plan -->
                    @if($assignment && $assignment->care_plan_id)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h2 class="text-lg font-bold text-gray-900 mb-4">Active Care Plan</h2>
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p class="text-sm text-blue-800">
                                <span class="font-semibold">Care Plan ID:</span> {{ $assignment->care_plan_id }}
                            </p>
                            <p class="text-xs text-blue-600 mt-2">Care plan details will be displayed here when available.</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Allergies -->
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            <h2 class="text-lg font-bold text-gray-900 mb-4">Allergies</h2>
                            @if($patient->allergies)
                            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                <p class="text-sm text-red-800">{{ $patient->allergies }}</p>
                            </div>
                            @else
                        ...
                        <!-- Current Medications -->
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            <h2 class="text-lg font-bold text-gray-900 mb-4">Current Medications</h2>
                            @if($patient->current_medications)
                            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                <p class="text-sm text-yellow-800">{{ $patient->current_medications }}</p>
                            </div>
                            @else
                    ...
                    <!-- Last 7 Days Vital Signs -->
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h2 class="text-lg font-bold text-gray-900 mb-4">Vital Signs (Last 7 Days)</h2>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-purple-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-bold text-purple-700 uppercase tracking-wider">Temperature (°C)</th>
                                        <th class="px-6 py-3 text-left text-xs font-bold text-purple-700 uppercase tracking-wider">Heart Rate</th>
                                        <th class="px-6 py-3 text-left text-xs font-bold text-purple-700 uppercase tracking-wider">Weight (kg)</th>
                                        <th class="px-6 py-3 text-left text-xs font-bold text-purple-700 uppercase tracking-wider">Recorded By</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            ...
                                            <span class="text-xs text-gray-500">{{ $vital->created_at->format('h:i A') }}</span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            @if($vital->caregiver)
                                                <span class="text-purple-600">Caregiver</span>
                                            @elseif($vital->nurse)
                                                <span class="text-blue-600">Nurse</span>
                                            @else

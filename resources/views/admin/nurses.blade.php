...
<body class="bg-gray-100 min-h-screen" x-data="{ sidebarOpen: false, pageLoading: false }">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <!-- Top Header -->
            @include('admin.shared.header', ['title' => 'Nurses'])
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-purple-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <p class="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-1.5">Total Nurses</p>
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $nurses->total() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-emerald-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $nurses->where('is_active', true)->count() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-gray-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $nurses->where('is_active', false)->count() }}</p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                    ...
                    <form method="GET" action="{{ admin_route('admin.nurses') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
                        <!-- Search -->
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ admin_route('admin.nurses') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                                    ...
                                    <div class="flex-1 flex items-center gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 mb-1">
                                                <h3 class="text-sm font-semibold text-gray-900">{{ $nurse->name }}</h3>
                                                @if($nurse->is_active)
                                                ...
                                                @endif
                                                <span class="px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full text-xs font-semibold">{{ $nurse->consultations_count ?? 0 }} patients</span>
                                            </div>
                                            <p class="text-xs text-gray-600">{{ $nurse->email }} • {{ $nurse->phone ?? 'No phone' }}</p>
                                        </div>
                                    </div>
                                ...
                                <div class="p-5 space-y-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-900">{{ $nurse->email }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Phone</p>
                                            <p class="text-xs text-gray-900">{{ $nurse->phone ?? 'N/A' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Patients Attended</p>
                                            <p class="text-xs text-gray-900 font-semibold">{{ $nurse->consultations_count ?? 0 }}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-900">{{ $nurse->created_at->format('M d, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Last Login</p>
                                            <p class="text-xs text-gray-900">{{ $nurse->last_login_at ? $nurse->last_login_at->format('M d, Y H:i') : 'Never' }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="pt-3 border-t border-gray-200 flex flex-wrap gap-2">
                                        ...
                                        <button onclick="toggleStatus({{ $nurse->id }}, {{ $nurse->is_active ? 'false' : 'true' }})" 
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white {{ $nurse->is_active ? 'bg-gray-600' : 'bg-emerald-600' }} rounded-lg hover:opacity-90 transition">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                            ...
                            <h3 class="text-sm font-semibold text-gray-900 mb-2">No Nurses Found</h3>
                        </div>
                    @endforelse
    ...
    <script>
        ...
        function openAddModal() {
            ...
            document.getElementById('form').action = '{{ admin_route('admin.nurses.store') }}';

...
<body class="bg-gray-100 min-h-screen" x-data="{ sidebarOpen: false }">
                ...
                <div class="flex items-center justify-between px-6 py-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <h1 class="text-xl font-bold text-white">Canvassers</h1>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-purple-200 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-xs font-medium uppercase tracking-wide mb-1">Total Canvassers</p>
                                <p class="text-2xl font-bold text-purple-600">{{ $canvassers->total() }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-emerald-200 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-emerald-600">{{ $canvassers->where('is_active', true)->count() }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-600">{{ $canvassers->where('is_active', false)->count() }}</p>
                            </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-purple-600 text-white">
                                <tr>
                                    ...
                                    <th class="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wide">Patients Registered</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span class="text-sm font-semibold text-gray-800">{{ $canvasser->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $canvasser->email }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $canvasser->phone ?? 'N/A' }}</td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-lg text-sm font-bold">{{ $canvasser->consultations_count ?? 0 }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-600">{{ $canvasser->created_at->format('M d, Y') }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-600">
                                        {{ $canvasser->last_login_at ? $canvasser->last_login_at->format('M d, Y H:i') : 'Never' }}
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button onclick="toggleStatus({{ $canvasser->id }}, {{ $canvasser->is_active ? 'false' : 'true' }})" 
                                                    class="px-3 py-1.5 {{ $canvasser->is_active ? 'bg-gray-600' : 'bg-emerald-600' }} text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-all">
                                ...
                                <tr>
                                    <td colspan="9" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p class="text-xl font-semibold">No canvassers found</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        ...
        async function toggleStatus(id, newStatus) {
            if (!confirm(`Are you sure you want to ${newStatus ? 'activate' : 'deactivate'} this canvasser?`)) {
                return;
            }

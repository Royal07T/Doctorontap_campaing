...
<body class="bg-gray-100 min-h-screen" x-data="{ sidebarOpen: false, pageLoading: false }">
    <div class="flex h-screen overflow-hidden">
        @include('admin.shared.sidebar', ['active' => 'care-givers'])

        <!-- Main Content Area -->
        <div class="flex-1 flex flex-col overflow-hidden">
            <!-- Top Header -->
            @include('admin.shared.header', ['title' => 'Care Givers'])
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-purple-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <p class="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-1.5">Total Care Givers</p>
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $careGivers->total() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-emerald-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $careGivers->where('is_active', true)->count() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-gray-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $careGivers->where('is_active', false)->count() }}</p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                    ...
                    <form method="GET" action="{{ route('admin.care-givers') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
                        <!-- Search -->
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.care-givers') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                                    ...
                                    <div class="flex-1 flex items-center gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 mb-1">
                                                <h3 class="text-sm font-semibold text-gray-900">{{ $careGiver->name }}</h3>
                                                ...
                                                @endif
                                                <span class="px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full text-xs font-semibold">{{ $careGiver->consultations_count ?? 0 }} consultations</span>
                                            </div>
                                            <p class="text-xs text-gray-600">{{ $careGiver->email }} • {{ $careGiver->phone ?? 'No phone' }}</p>
                                        </div>
                                    </div>
                                ...
                                <div class="p-5 space-y-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-900">{{ $careGiver->created_at->format('M d, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Last Login</p>
                                            <p class="text-xs text-gray-900">{{ $careGiver->last_login_at ? $careGiver->last_login_at->format('M d, Y H:i') : 'Never' }}</p>
                                        </div>
                                    </div>

                                    <!-- Uploaded Documents -->
                                    @if($careGiver->cv_path || $careGiver->profile_photo_path)
                                        <div class="pt-3 border-t border-gray-200">
                                            <p class="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">Uploaded Documents</p>
                                            <div class="flex flex-wrap gap-2">
                                                @if($careGiver->cv_path)
                                                    <a href="{{ route('admin.care-givers.cv', $careGiver->id) }}" target="_blank"
                                                ...
                                                @if($careGiver->profile_photo_path)
                                                    <a href="{{ route('admin.care-givers.photo', $careGiver->id) }}" target="_blank"
                    ...
                    @empty
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                            ...
                            <h3 class="text-sm font-semibold text-gray-900 mb-2">No Care Givers Found</h3>
                        </div>
                    @endforelse
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm hidden items-center justify-center z-50" style="display: none;">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div class="purple-gradient text-white px-6 py-4 flex items-center justify-between rounded-t-xl">
                <h2 id="modalTitle" class="text-xl font-bold">Add New Care Giver</h2>
            ...
            <form id="form" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm font-medium text-gray-700">Care Giver is active</span>
                    </label>
                </div>
                ...
                <div class="flex gap-3 mt-6 pt-4 border-t border-gray-200">
                    ...
                    <button type="submit" id="submitBtn"
                        ...
                        <span id="submitBtnText">Create Care Giver</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function openAddModal() {
            ...
            document.getElementById('form').action = '{{ route("admin.care-givers.store") }}';
            document.getElementById('submitBtnText').textContent = 'Create Care Giver';
        ...
        function openEditModal(record) {
            document.getElementById('modalTitle').textContent = 'Edit Care Giver';
            ...
            document.getElementById('form').action = `/admin/care-givers/${record.id}`;
            document.getElementById('submitBtnText').textContent = 'Update Care Giver';
            document.getElementById('formMessage').classList.add('hidden');
            document.getElementById('modal').style.display = 'flex';
            document.body.style.overflow = 'hidden';
        }
        ...
        async function toggleStatus(id, newStatus) {
            showConfirmModal(
                `Are you sure you want to ${newStatus ? 'activate' : 'deactivate'} this care giver?`,
                async () => {
                    try {
                        const response = await fetch(`/admin/care-givers/${id}/toggle-status`, {
                            method: 'POST',
                            headers: {
        ...
        async function deleteRecord(id) {
            showConfirmModal(
                'Are you sure you want to delete this care giver? This action cannot be undone.',
                async () => {
                    try {
                        const response = await fetch(`/admin/care-givers/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',

...
<head>
    ...
    <title>Customer Care - Admin</title>
...
<body class="bg-gray-100 min-h-screen" x-data="{ sidebarOpen: false, pageLoading: false }">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <!-- Top Header -->
            @include('admin.shared.header', ['title' => 'Customer Care'])
            ...
            <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-purple-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <p class="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-1.5">Total Customer Care</p>
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $customerCares->total() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-emerald-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $customerCares->where('is_active', true)->count() }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 p-5 border-l-4 border-gray-500">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xl font-bold text-gray-900 mb-1">{{ $customerCares->where('is_active', false)->count() }}</p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                    ...
                    <form method="GET" action="{{ route('admin.customer-cares') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
                        <!-- Search -->
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.customer-cares') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Customer Care Cards -->
                <div class="space-y-4">
                    @forelse($customerCares as $customerCare)
                                    ...
                                    <div class="flex-1 flex items-center gap-3">
                                        <div class="flex-shrink-0">
                                            <div class="w-10 h-10 rounded-full bg-purple-600 flex items-center justify-center text-white font-bold text-xs">
                                                {{ substr($customerCare->name, 0, 1) }}
                                            </div>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 mb-1">
                                                <h3 class="text-sm font-semibold text-gray-900">{{ $customerCare->name }}</h3>
                                                ...
                                                @endif
                                                <span class="px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full text-xs font-semibold">{{ $customerCare->consultations_count ?? 0 }} consultations</span>
                                            </div>
                                            <p class="text-xs text-gray-600">{{ $customerCare->email }} • {{ $customerCare->phone ?? 'No phone' }}</p>
                                        </div>
                                    </div>
                                ...
                                <div class="p-5 space-y-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-900">{{ $customerCare->email }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Phone</p>
                                            <p class="text-xs text-gray-900">{{ $customerCare->phone ?? 'N/A' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Consultations</p>
                                            <p class="text-xs text-gray-900 font-semibold">{{ $customerCare->consultations_count ?? 0 }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Created By</p>
                                            <p class="text-xs text-gray-900">
                                                @if($customerCare->createdBy)
                                                    <span class="text-purple-600 font-medium">{{ $customerCare->createdBy->name }}</span>
                                                @else
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-900">{{ $customerCare->created_at->format('M d, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">Last Login</p>
                                            <p class="text-xs text-gray-900">{{ $customerCare->last_login_at ? $customerCare->last_login_at->format('M d, Y H:i') : 'Never' }}</p>
                                        </div>
                                    </div>

                                    <!-- Action Buttons -->
                                    <div class="pt-3 border-t border-gray-200 flex flex-wrap gap-2">
                                        <a href="{{ route('admin.customer-care-oversight.agents.show', $customerCare) }}" 
                                        ...
                                        <button onclick='openEditModal(@json($customerCare))' 
                                        ...
                                        <button onclick="toggleStatus({{ $customerCare->id }}, {{ $customerCare->is_active ? 'false' : 'true' }})" 
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white {{ $customerCare->is_active ? 'bg-gray-600' : 'bg-emerald-600' }} rounded-lg hover:opacity-90 transition">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                            {{ $customerCare->is_active ? 'Deactivate' : 'Activate' }}
                                        </button>
                                        <button onclick="deleteRecord({{ $customerCare->id }})" 
                    ...
                    @empty
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                            ...
                            <h3 class="text-sm font-semibold text-gray-900 mb-2">No Customer Care Found</h3>
                        </div>
                    @endforelse

                    <!-- Pagination -->
                    @if($customerCares->hasPages())
                    <div class="mt-6">
                        {{ $customerCares->links() }}
                    </div>
                    @endif
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm hidden items-center justify-center z-50" style="display: none;">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div class="purple-gradient text-white px-6 py-4 flex items-center justify-between rounded-t-xl">
                <h2 id="modalTitle" class="text-xl font-bold">Add New Customer Care</h2>
            ...
            <form id="form" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm font-medium text-gray-700">Customer Care is active</span>
                    </label>
                </div>
                ...
                <div class="flex gap-3 mt-6 pt-4 border-t border-gray-200">
                    ...
                    <button type="submit" id="submitBtn"
                        ...
                        <span id="submitBtnText">Create Customer Care</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function openAddModal() {
            document.getElementById('modalTitle').textContent = 'Add New Customer Care';
            ...
            document.getElementById('form').action = '{{ route("admin.customer-cares.store") }}';
            document.getElementById('submitBtnText').textContent = 'Create Customer Care';
        ...
        function openEditModal(record) {
            document.getElementById('modalTitle').textContent = 'Edit Customer Care';
            ...
            document.getElementById('form').action = `/admin/customer-cares/${record.id}`;
            document.getElementById('submitBtnText').textContent = 'Update Customer Care';
            document.getElementById('formMessage').classList.add('hidden');
            document.getElementById('modal').style.display = 'flex';
            document.body.style.overflow = 'hidden';
        }
        ...
        async function toggleStatus(id, newStatus) {
            showConfirmModal(
                `Are you sure you want to ${newStatus ? 'activate' : 'deactivate'} this customer care?`,
                async () => {
                    try {
                        const response = await fetch(`/admin/customer-cares/${id}/toggle-status`, {
                            method: 'POST',
                            headers: {
        ...
        async function deleteRecord(id) {
            showConfirmModal(
                'Are you sure you want to delete this customer care? This action cannot be undone.',
                async () => {
                    try {
                        const response = await fetch(`/admin/customer-cares/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        document.getElementById('form').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Get the method from the hidden _method field
                const method = formData.get('_method') || 'POST';
                
                // If it's PUT, we need to use POST with _method override
                ...
                try {
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        throw new Error('Server returned an unexpected response. Please check the form and try again.');
                    }
                } catch (parseError) {
                    if (parseError.message.includes('unexpected response')) {
                        throw parseError;
                    }
                    // If JSON parsing fails, it might be an HTML error page
                    console.error('JSON parse error:', parseError);
                    throw new Error('Failed to parse server response. Please check your input and try again.');
                }
                
                if (response.ok && data.success) {
                    formMessage.className = 'mb-4 p-3 rounded-lg bg-green-100 text-green-800 border border-green-200';
                    formMessage.textContent = data.message || 'Customer Care saved successfully!';
                    ...
                    submitBtnText.textContent = document.getElementById('recordId').value ? 'Update Customer Care' : 'Create Customer Care';
                }
            } catch (error) {
                console.error('Error:', error);
                formMessage.className = 'mb-4 p-3 rounded-lg bg-red-100 text-red-800 border border-red-200';

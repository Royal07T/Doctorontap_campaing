...
<body class="min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-teal-100 mt-1 text-sm">Doctor Portal</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-xl p-6">
            ...
            <div class="text-center mb-6">
                ...
                <p class="text-xs text-gray-500">
                    Email: <strong>{{ Auth::guard('doctor')->user()->email }}</strong>
                </p>
            </div>

            <!-- Resend Button -->
            <form method="POST" action="{{ route('doctor.verification.resend') }}">
                @csrf
                <button type="submit" 
            ...
            <div class="mt-4 text-center">
                <form method="POST" action="{{ route('doctor.logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="text-xs text-teal-600 hover:text-teal-800 font-semibold">
                        Logout
                    </button>
                </form>
            </div>
        ...
        <div class="mt-4 text-center">
            <div class="inline-flex items-center gap-2 bg-teal-900 bg-opacity-30 px-4 py-2 rounded-lg">
                <svg class="w-4 h-4 text-teal-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">

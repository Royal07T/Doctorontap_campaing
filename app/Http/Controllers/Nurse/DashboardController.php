...
public function searchPatients(Request $request)
    ...
    /**
     * Show walk-in vital signs form
     */
    public function showWalkInForm()
    {
        return view('nurse.walk-in-vitals');
    }

    /**
     * Store walk-in vital signs (for events like fun fair)
     */
    public function storeWalkInVitals(Request $request)
    {
        try {
            $validated = $request->validate([
            ...
            return response()->json([
                'success' => true,
                'message' => 'Vital signs recorded and report sent to ' . $patient->email . ' successfully!'
            ]);

        } catch (\Exception $e) {
            \Log::error('Failed to record walk-in vital signs: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Failed to process walk-in vital signs: ' . $e->getMessage()
            ], 500);
        }
    }
}

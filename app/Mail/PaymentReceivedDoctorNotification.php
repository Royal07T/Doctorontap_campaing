...
class PaymentReceivedDoctorNotification extends Mailable implements ShouldQueue
{
    ...
    public function __construct(Consultation $consultation, Payment $payment)
    {
        ...
        // Calculate doctor portion (typically 30% of total)
        $doctorPortion = ($payment->amount ?? 0) * 0.30;
        ...
        $templateData = [
            ...
            'doctor_portion' => number_format($doctorPortion, 2),
        ];

        // Try to get template from CommunicationTemplate system
        $rendered = EmailTemplateService::render('PaymentReceivedDoctorNotification', $templateData);
